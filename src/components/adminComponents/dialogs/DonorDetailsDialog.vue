...
export default {
  methods: {
    closeDialog() {
      this.$store.commit("adminModule/showDonorDetailDialog", false);
    }
  },
  computed: {
    donorProfile() {
      return this.$store.state.adminModule.dialogs.donorDetailsDialog
        .donorProfile;
    },
    showDialog: {
      get() {
      ...
      set(value) {
        this.$store.commit("adminModule/showDonorDetailDialog", value);
      }
    }
  },
  updated() {
    this.$store.commit("adminModule/initalizeDonorProfileMap");
    this.$store.commit("adminModule/changeDonorDetailsLocation", {
